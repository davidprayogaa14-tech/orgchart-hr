...
#   Employment Status                     → (filter: active only, then drop)
#
...
_PEOPLE_DB_DROP_COLS = [
    ...
    "HRBP Email", "Join Date", "Employment Status",
]
...
def normalize_people_db(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Step 2: Filter karyawan aktif ─────────────────────────────
    if "Employment Status" in df.columns:
        df["Employment Status"] = df["Employment Status"].astype(str).str.strip()
        active_mask = df["Employment Status"].str.lower().isin(_ACTIVE_STATUS_VALUES)
        total_before = len(df)
        df = df[active_mask].copy()
        total_after  = len(df)
        dropped = total_before - total_after
        if dropped > 0:
            # Log ke console — bukan ke UI agar tidak mengganggu user
            print(f"[normalize_people_db] Filtered {dropped} inactive/resigned employees "
                  f"({total_before} → {total_after} active records)")
