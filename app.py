...
def load_survey_responses():
    ...
    q_cols = [c for c in df.columns if c.endswith("_score")]
    df["engagement_score"] = df[q_cols].mean(axis=1).round(2)
    return df, "demo"


SURVEY_QUESTIONS = {
    "default": [
        "Saya memahami tujuan dan arah perusahaan",
        "Pekerjaan saya bermakna dan memberikan dampak nyata",
        "Saya mendapat dukungan yang cukup dari atasan langsung",
        "Ada kesempatan bagi saya untuk tumbuh dan berkembang",
        "Kolaborasi antar tim berjalan dengan baik",
        "Saya merasa diakui atas kontribusi yang saya berikan",
        "Saya memiliki alat dan sumber daya yang dibutuhkan",
        "Secara keseluruhan, saya puas bekerja di sini",
    ]
}
